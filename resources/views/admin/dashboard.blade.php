    ...
    @push('javascript')
        <script>
            ...
            var myChart = new Chart(ctx, {
                type: 'bar',
                data: {
                    labels: ['Jab', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', "Sep", 'Oct', 'Nov', 'Dec'],
                    datasets: [{
                        label: '# of Users',
                        data: [0, 0, 0, 1, 47, 3, 0, 0, 0, 0 , 0 , 0],
                        backgroundColor: [
                            'rgb(24, 228, 140)',
                        ],
                        borderColor: [
            ...
            jQuery(document).ready(function($) {
                var range = {
                    ...
                    "Last_Month": [moment().subtract(1, 'month').startOf('month'), moment().subtract(1, 'month')
                        .endOf('month')
                    ],
            ...
            if (!empty($_GET['range']) && in_array($_GET['range'], array('Today','Yesterday','This Week','This Month','Last Month','This Year'))) { 
                ...
                $(document).on('click', '.applyBtn', function(event) {
                    ...
                    $('#redirect_link').attr('href', "&range=" + $(this).parent('.drp-buttons').find(
                        '.drp-selected').html());
                    $('#redirect_link').attr('data-ajax', "?path=dashboard&range=" + $(this).parent(
                        '.drp-buttons').find('.drp-selected').html());
                    $('#redirect_link').click();
                });
