...
<div class="live_vid d-none">
    <div class="btns d-none _liveBtn d-flex">
        ...
        <button class="btn bg-primary d-none" id="speed_con">Connecting User <div
                class="spinner-border spinner-border-sm mx-2 text-white" style=""></div></button>
        <button onclick="leaveAndRemoveLocalStream()" class="btn bg-danger" id="leave-btn">Leave <i
                class="fa-solid fa-person-walking-arrow-right"></i></button>
        <button onclick="toggleMic()" class="btn bg-danger" id="mic-btn">Mic On <i
                class="fa-solid fa-microphone"></i></button>
        <button onclick="toggleCamera()" class="btn bg-danger" id="camera-btn">Camera On <i
                class="fa-solid fa-camera-retro"></i></button>


        <button data-bs-toggle="offcanvas" data-bs-target="#messages_" class="btn"
            style="background-color: rgb(133, 133, 134)">Messages <i class="fa-solid fa-message"></i></button>
        ...
        <button data-bs-toggle="offcanvas" id="_mus" data-bs-target="#music_" class="btn d-none"
            style="background-color: rgb(71, 202, 130)">Music <i class="fa-solid fa-music"></i></button>
                                ...
                                @foreach ($audio as $item)
                                    <div class="d-flex" style="border-bottom:1px solid lightgray">
                                        <div>
                                            ...
                                            <p style="font-size: 10px" class="name mb-0 text-capitalize">Music by
                                                {{ $item->artist }}
                                                <span style="font-size: 16px" class="mb-0 text-danger playbtn mx-2"
                                                    onclick="playAudio('{{ url('audio/' . $item->filename) }}')"><i
                                                        style="font-size: 20px !important"
                                                        class="fa-regular fa-circle-play"></i></span>
                                                <span style="font-size: 16px" class="mb-0 text-info "
                                                    onclick="pauseAudio('{{ url('audio/' . $item->filename) }}')"><i
                                                        style="font-size: 20px !important"
                                                        class="fa-regular fa-circle-pause"></i></span>
                                            </p>
                                        </div>
                                    </div>
                                @endforeach
                    ...
                    @foreach ($music_type as $music)
                        <div class="card" style="box-shadow: .1em .1em .5em rgb(240, 240, 240)">
                            <div class="card-header bg-dark" data-bs-toggle="collapse"
                                href="#{{ $music }}collapseOne">
                                <a class="collapsed text-white">
                                    {{ $music }}
                                </a>
                            </div>
                            <div id="{{ $music }}collapseOne" class="collapse" data-bs-parent="#accordion">
                                <div class="card-body">
                                    @foreach ($audio as $item)
                                        @if (ucfirst($item->type) == $music)
                                            <div class="d-flex" style="border-bottom:1px solid lightgray">
                                                <div>
                                                    <h6 class="name mb-0 text-capitalize">{{ $item->name }}</h6>
                                                    <p style="font-size: 10px" class="name mb-0 text-capitalize">Music
                                                        by {{ $item->artist }}
                                                        <span style="font-size: 16px"
                                                            class="mb-0 text-danger playbtn mx-2"
                                                            onclick="playAudio('{{ url('audio/' . $item->filename) }}')"><i
                                                                style="font-size: 20px !important"
                                                                class="fa-regular fa-circle-play"></i></span>
                                                        <span style="font-size: 16px" class="mb-0 text-info "
                        ...
                        <div class="d-flex mt-5">
                            <div>
                                <p style="font-size: 14px" class="name mb-0 text-capitalize">Play all music
                                    <span style="font-size: 16px" class="mb-0 text-danger playbtn mx-2"
                                        onclick="playAudio('{{ url('audio/' . $audio[0]->filename) }}')"><i
                                            style="font-size: 20px !important"
                                            class="fa-regular fa-circle-play"></i></span>
                                    <span style="font-size: 16px" class="mb-0 text-info "
                                        onclick="pauseAudio('{{ url('audio/' . $audio[0]->filename) }}')"><i
                                            style="font-size: 20px !important"
                                            class="fa-regular fa-circle-pause"></i></span>
                                </p>
                            </div>
